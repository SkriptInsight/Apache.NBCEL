	...
	public class PushbackInputStream : FilterInputStream
    {
	    ...
	    public override int Read(byte[] b, int off, int len)
        {
            ...
            if (len > 0)
            {
                ...
                if (len == -1) return avail == 0 ? -1 : avail;
                return avail + len;
            }
        ...
        {
            EnsureOpen();
            if (pos == 0) throw new IOException("Push back buffer is full");
            buf[--pos] = unchecked((byte) b);
        }
        ...
        {
            EnsureOpen();
            if (len > pos) throw new IOException("Push back buffer is full");
            pos -= len;
	    ...
	    ///         <see cref="FilterInputStream.Available()">available</see>
	    ///         .
	    /// </remarks>
	    /// <returns>
	    ...
	    /// <seealso cref="InputStream.Available()" />
	    /// <exception cref="System.IO.IOException" />
	    public override int Available()
        {
            EnsureOpen();
            var n = buf.Length - pos;
	    ...
	    /// </exception>
	    /// <seealso cref="FilterInputStream.@in" />
	    /// <seealso cref="InputStream.Skip(long)" />
	    /// <since>1.2</since>
	    /// <exception cref="System.IO.IOException" />
