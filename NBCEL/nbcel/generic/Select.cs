...
{
	/// <summary>Select - Abstract super class for LOOKUPSWITCH and TABLESWITCH instructions.
	/// 	</summary>
	/// <remarks>
	...
	public abstract class Select : NBCEL.generic.BranchInstruction, NBCEL.generic.VariableLengthInstruction
		, NBCEL.generic.StackConsumer, NBCEL.generic.StackProducer
	{
		...
		protected internal NBCEL.generic.InstructionHandle[] targets;
		...
		protected internal int padding = 0;
		...
		internal Select(short opcode, int[] match, NBCEL.generic.InstructionHandle[] targets
			, NBCEL.generic.InstructionHandle defaultTarget)
			: base(opcode, null)
		{
			/* @since 6.0 */
			...
			foreach (NBCEL.generic.InstructionHandle target2 in targets)
			{
				NotifyTarget(null, target2, this);
			}
			if ((match_length = match.Length) != targets.Length)
			{
				throw new NBCEL.generic.ClassGenException("Match and target array have not the same length: Match length: "
		...
		protected internal override int UpdatePosition(int offset, int max_offset)
		{
			...
			*/
			...
			base.SetLength((short)(fixed_length + padding));
			// Update length
		...
		{
			System.Text.StringBuilder buf = new System.Text.StringBuilder(base.ToString(verbose
				));
			if (verbose)
			{
		...
		public virtual void SetTarget(int i, NBCEL.generic.InstructionHandle target)
		{
		...
		public override void UpdateTarget(NBCEL.generic.InstructionHandle old_ih, NBCEL.generic.InstructionHandle
			 new_ih)
		{
			bool targeted = false;
			...
			if (!targeted)
			{
				throw new NBCEL.generic.ClassGenException("Not targeting " + old_ih);
			}
		}

		/// <returns>true, if ih is target of this instruction</returns>
		public override bool ContainsTarget(NBCEL.generic.InstructionHandle ih)
		{
			if (base.GetTarget() == ih)
			{
				return true;
			}
			foreach (NBCEL.generic.InstructionHandle target2 in targets)
		...
		{
			NBCEL.generic.Select copy = (NBCEL.generic.Select)base.MemberwiseClone();
		...
		public virtual NBCEL.generic.InstructionHandle[] GetTargets()
		{
			return targets;
		}
		...
		/// <since>6.0</since>
		internal NBCEL.generic.InstructionHandle GetTarget(int index)
		{
		...
		/// <param name="array"/>
		/// <since>6.0</since>
		internal void SetTargets(NBCEL.generic.InstructionHandle[] array)
		{
			targets = array;
		}
