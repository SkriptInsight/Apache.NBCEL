...
namespace NBCEL.util
{
	...
	/// the Code frame.
	/// </remarks>
	public class Class2HTML : NBCEL.Constants
	{
		private readonly NBCEL.classfile.JavaClass java_class;
		...
		private static NBCEL.classfile.ConstantPool constant_pool;

		private static readonly System.Collections.Generic.HashSet<string> basic_types = 
		...
		public Class2HTML(NBCEL.classfile.JavaClass java_class, string dir)
		{
			NBCEL.classfile.Method[] methods = java_class.GetMethods();
			...
			if (index > -1)
			{
				class_package = Sharpen.Runtime.Substring(class_name, 0, index);
			}
			else
			{
			...
			// default package
			NBCEL.util.ConstantHTML constant_html = new NBCEL.util.ConstantHTML(dir, class_name
			...
			*/
			NBCEL.util.AttributeHTML attribute_html = new NBCEL.util.AttributeHTML(dir, class_name
				, constant_pool, constant_html);
			new NBCEL.util.MethodHTML(dir, class_name, methods, java_class.GetFields(), constant_html
				, attribute_html);
			// Write main file (with frames, yuk)
			WriteMainHTML(attribute_html);
			new NBCEL.util.CodeHTML(dir, class_name, methods, constant_pool, constant_html);
		...
		public static void Main(string[] argv)
		{
			...
			int files = 0;
			NBCEL.classfile.ClassParser parser = null;
			NBCEL.classfile.JavaClass java_class = null;
			...
			*/
			for (int i = 0; i < argv.Length; i++)
			{
				if (argv[i][0] == '-')
				{
					...
					if (argv[i].Equals("-d"))
					{
						...
						dir = argv[++i];
						if (!dir.EndsWith(string.Empty + sep))
						{
							dir = dir + sep;
						}
					...
					{
						System.Console.Out.WriteLine("Unknown option " + argv[i]);
					}
				}
				else
				{
			...
			if (files == 0)
			{
				System.Console.Error.WriteLine("Class2HTML: No input files specified.");
			}
			else
			{
				// Loop through files ...
				...
				{
					System.Console.Out.Write("Processing " + file_name[i] + "...");
					if (zip_file == null)
					{
						parser = new NBCEL.classfile.ClassParser(file_name[i]);
					}
					else
					{
						// Create parser object from file
						parser = new NBCEL.classfile.ClassParser(zip_file, file_name[i]);
					}
					// Create parser object from zip file
					java_class = parser.Parse();
					new NBCEL.util.Class2HTML(java_class, dir);
		...
		{
			string str = constant_pool.GetConstantString(index, NBCEL.Const.CONSTANT_Class);
			str = NBCEL.classfile.Utility.CompactClassName(str);
			str = NBCEL.classfile.Utility.CompactClassName(str, class_package + ".", true);
		...
		{
			string short_type = NBCEL.classfile.Utility.CompactClassName(type);
			short_type = NBCEL.classfile.Utility.CompactClassName(short_type, class_package +
				 ".", true);
			...
			if (index > -1)
			{
				base_type = Sharpen.Runtime.Substring(type, 0, index);
			}
			// Tack of the `['
			// test for basic type
		...
		/// <exception cref="System.IO.IOException"/>
		private void WriteMainHTML(NBCEL.util.AttributeHTML attribute_html)
		{
			using (System.IO.TextWriter file = new System.IO.StreamWriter(File.OpenWrite(dir + class_name + ".html")))
			{
			...
			NBCEL.classfile.Attribute[] attributes = java_class.GetAttributes();
