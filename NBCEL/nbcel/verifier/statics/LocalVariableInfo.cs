...
namespace NBCEL.verifier.statics
{
	...
	public class LocalVariableInfo
	{
		...
		private readonly Dictionary<string, NBCEL.generic.Type> types = new Dictionary
			<string, NBCEL.generic.Type>();
		...
		/// (Hashtable) database.
		/// </summary>
		private void SetType(int offset, NBCEL.generic.Type t)
		{
			types[offset.ToString()] = t;
		}

		/// <summary>Returns the type of the local variable that uses this local variable slot at the given bytecode offset.
		/// 	</summary>
		/// <remarks>
		...
		/// 	</returns>
		public virtual NBCEL.generic.Type GetType(int offset)
		{
			return types.GetOrNull(offset.ToString());
		}

		/// <summary>Returns the name of the local variable that uses this local variable slot at the given bytecode offset.
		/// 	</summary>
		/// <remarks>
		...
		/// </exception>
		public virtual void Add(string name, int startPc, int length, NBCEL.generic.Type 
			type)
		{
		...
		/// with already gathered information.
		/// </exception>
		private void Add(int offset, string name, NBCEL.generic.Type t)
		{
			if (GetName(offset) != null)
			{
				if (!GetName(offset).Equals(name))
				{
					throw new NBCEL.verifier.exc.LocalVariableInfoInconsistentException("At bytecode offset '"
						 + offset + "' a local variable has two different names: '" + GetName(offset) + 
						"' and '" + name + "'.");
				}
			}
			if (GetType(offset) != null)
			{
				...
				{
					throw new NBCEL.verifier.exc.LocalVariableInfoInconsistentException("At bytecode offset '"
						 + offset + "' a local variable has two different types: '" + GetType(offset) + 
						"' and '" + t + "'.");
				}
			}
			SetName(offset, name);
