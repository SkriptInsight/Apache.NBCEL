...
namespace NBCEL.verifier.statics
{
	...
	public sealed class Pass1Verifier : NBCEL.verifier.PassVerifier
	{
		...
		/// </remarks>
		private NBCEL.classfile.JavaClass GetJavaClass()
		{
			if (jc == null)
			{
				try
		...
		public Pass1Verifier(NBCEL.verifier.Verifier owner)
		{
			myOwner = owner;
		}
		...
		public override NBCEL.verifier.VerificationResult Do_verify()
		{
			...
			try
			{
				...
				if (jc != null)
				{
					...
					if (!myOwner.GetClassName().Equals(jc.GetClassName()))
					{
						...
						throw new NBCEL.verifier.exc.LoadingException("Wrong name: the internal name of the .class file '"
							 + jc.GetClassName() + "' does not match the file's name '" + myOwner.GetClassName
							() + "'.");
					}
				}
			}
			catch (NBCEL.verifier.exc.LoadingException e)
			{
				return new NBCEL.verifier.VerificationResult(NBCEL.verifier.VerificationResult.VERIFIED_REJECTED
					, e.Message);
			}
			catch (NBCEL.classfile.ClassFormatException e)
			{
				return new NBCEL.verifier.VerificationResult(NBCEL.verifier.VerificationResult.VERIFIED_REJECTED
					, e.Message);
			}
			catch (System.Exception e)
			{
				...
				return new NBCEL.verifier.VerificationResult(NBCEL.verifier.VerificationResult.VERIFIED_REJECTED
					...
					NBCEL.verifier.exc.Utility.GetStackTrace(e));
			}
			if (jc != null)
			{
