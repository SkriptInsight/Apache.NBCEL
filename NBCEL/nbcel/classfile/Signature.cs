...
namespace NBCEL.classfile
{
	...
	public sealed class Signature : NBCEL.classfile.Attribute
	{
		...
		/// </remarks>
		public Signature(NBCEL.classfile.Signature c)
		...
		internal Signature(int name_index, int length, java.io.DataInput input, NBCEL.classfile.ConstantPool
			 constant_pool)
		...
		/// <param name="constant_pool">Array of constants</param>
		public Signature(int name_index, int length, int signature_index, NBCEL.classfile.ConstantPool
			 constant_pool)
			: base(NBCEL.Const.ATTR_SIGNATURE, name_index, length, constant_pool)
		{
		...
		public string GetSignature()
		{
			NBCEL.classfile.ConstantUtf8 c = (NBCEL.classfile.ConstantUtf8)base.GetConstantPool
				().GetConstant(signature_index, NBCEL.Const.CONSTANT_Utf8);
			return c.GetBytes();
		}
		...
		private sealed class MyByteArrayInputStream : MemoryInputStream
		{
			internal MyByteArrayInputStream(string data)
				: base(Sharpen.Runtime.GetBytesForString(data))
			{
			}

			internal string GetData()
			{
				return Sharpen.Runtime.GetStringForBytes(Stream.GetBuffer());
			}
		...
		private static void MatchIdent(NBCEL.classfile.Signature.MyByteArrayInputStream @in
			, System.Text.StringBuilder buf)
		{
			int ch;
			if ((ch = @in.Read()) == -1)
			{
				throw new System.Exception("Illegal signature: " + @in.GetData() + " no ident, reaching EOF"
					);
			}
			//System.out.println("return from ident:" + (char)ch);
			if (!IdentStart(ch))
			{
				System.Text.StringBuilder buf2 = new System.Text.StringBuilder();
				int count = 1;
				while (Sharpen.Runtime.IsJavaIdentifierPart((char)ch))
				{
					buf2.Append((char)ch);
					count++;
					ch = @in.Read();
			...
			System.Text.StringBuilder buf2_1 = new System.Text.StringBuilder();
			ch = @in.Read();
			do
			{
			...
			while ((ch != -1) && (Sharpen.Runtime.IsJavaIdentifierPart((char)ch) || (ch == '/')));
		...
		private static void MatchGJIdent(NBCEL.classfile.Signature.MyByteArrayInputStream
			 @in, System.Text.StringBuilder buf)
		{
			int ch;
			MatchIdent(@in, buf);
			ch = @in.Read();
			if ((ch == '<') || ch == '(')
			{
				...
				while (((ch = @in.Read()) != '>') && (ch != ')'))
				{
					...
					if (ch == -1)
					{
						throw new System.Exception("Illegal signature: " + @in.GetData() + " reaching EOF"
							);
					}
			...
			{
				throw new System.Exception("Illegal signature: " + @in.GetData() + " read " + (char
					)ch);
			}
		}
		...
		public static string Translate(string s)
		{
			...
			MatchGJIdent(new NBCEL.classfile.Signature.MyByteArrayInputStream(s), buf);
			return buf.ToString();
		}
