...
{
	/// <summary>This class is derived from <em>Attribute</em> and represents the list of packages that are exported or opened by the Module attribute.
	/// 	</summary>
	/// <remarks>
	...
	public sealed class ModulePackages : NBCEL.classfile.Attribute
	{
		...
		/// </remarks>
		public ModulePackages(NBCEL.classfile.ModulePackages c)
		...
		public ModulePackages(int name_index, int length, int[] package_index_table, NBCEL.classfile.ConstantPool
			 constant_pool)
			: base(NBCEL.Const.ATTR_MODULE_PACKAGES, name_index, length, constant_pool)
		{
			this.package_index_table = package_index_table != null ? package_index_table : new 
				int[0];
		}
		...
		internal ModulePackages(int name_index, int length, java.io.DataInput input, NBCEL.classfile.ConstantPool
			 constant_pool)
		...
		public override void Dump(java.io.DataOutputStream file)
		{
			...
			foreach (int index in package_index_table)
			{
		...
		{
			string[] names = new string[package_index_table.Length];
			for (int i = 0; i < package_index_table.Length; i++)
			{
				names[i] = base.GetConstantPool().GetConstantString(package_index_table[i], NBCEL.Const
		...
		public override string ToString()
		{
			...
			foreach (int index in package_index_table)
			{
				string package_name = base.GetConstantPool().GetConstantString(index, NBCEL.Const
					.CONSTANT_Package);
				buf.Append("  ").Append(NBCEL.classfile.Utility.CompactClassName(package_name, false
					)).Append("\n");
			}
		...
		{
			NBCEL.classfile.ModulePackages c = (NBCEL.classfile.ModulePackages)Clone();
			if (package_index_table != null)
			{
				c.package_index_table = new int[package_index_table.Length];
				System.Array.Copy(package_index_table, 0, c.package_index_table, 0, package_index_table
					.Length);
			}
