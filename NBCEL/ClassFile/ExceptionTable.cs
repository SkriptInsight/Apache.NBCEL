...
namespace Apache.NBCEL.ClassFile
{
	...
	/// </remarks>
	/// <seealso cref="Code" />
	public sealed class ExceptionTable : Attribute
    {
        ...
        public ExceptionTable(int name_index, int length, int[] exception_index_table, ConstantPool
            constant_pool)
            : base(Const.ATTR_EXCEPTIONS, name_index, length, constant_pool)
        {
            // constant pool
        ...
        internal ExceptionTable(int name_index, int length, DataInput input, ConstantPool
            constant_pool)
            : this(name_index, length, (int[]) null, constant_pool)
        {
            var number_of_exceptions = input.ReadUnsignedShort();
            exception_index_table = new int[number_of_exceptions];
            for (var i = 0; i < number_of_exceptions; i++) exception_index_table[i] = input.ReadUnsignedShort();
        }
        ...
        public override void Dump(DataOutputStream file)
        {
            ...
            foreach (var index in exception_index_table) file.WriteShort(index);
        }
        ...
        {
            var names = new string[exception_index_table.Length];
            for (var i = 0; i < exception_index_table.Length; i++)
                names[i] = GetConstantPool().GetConstantString(exception_index_table[i], Const
        ...
        public override string ToString()
        {
            ...
            for (var i = 0; i < exception_index_table.Length; i++)
            {
                str = GetConstantPool().GetConstantString(exception_index_table[i], Const
                    .CONSTANT_Class);
                buf.Append(Utility.CompactClassName(str, false));
                if (i < exception_index_table.Length - 1) buf.Append(", ");
            }
        ...
        {
            var c = (ExceptionTable) Clone();
            if (exception_index_table != null)
            {
                c.exception_index_table = new int[exception_index_table.Length];
                Array.Copy(exception_index_table, 0, c.exception_index_table, 0, exception_index_table
                    .Length);
            }
